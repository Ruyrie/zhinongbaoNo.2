...
/**
 * ============================================================
 * 【发文章 / Add Article】Contract（接口约定）
 * 约定内容：
 *   - View：由 Presenter 调用来弹提示、发布成功后关闭页面。
 *   - Presenter：由 View 调用来提交文章（标题、正文、封面/配图串、分类）。
 * 数据流向：View 收集输入 → 调 submit → Presenter 写入 ArticleRepository → 回调 View 提示并关页。
 * 配合的文件：View 实现 = AddArticleActivity；Presenter 实现 = AddArticlePresenter。
...
public interface AddArticleContract {
    ...
    // Presenter：View（用户点提交）用这个方法触发业务处理
    interface Presenter extends BasePresenter {
        void submit(String title, String content, String coverUri, String category); // 提交文章
    }
}
